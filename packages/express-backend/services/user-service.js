...
function updateUser(userId, updates) {
  const promise = userModel.findByIdAndUpdate(userId, updates, { new: true }).catch((err) => console.log(err));
  return promise;
}

// Delete a user entirely.
function deleteUser(userId) {
  const promise = userModel.findByIdAndDelete(userId).catch((err) => console.log(err));
  return promise;
}

// Get Venmo username for a user.
function getVenmo(userId) {
  const promise = userModel.findById(userId).select('venmo_username').catch((err) => console.log(err));
  return promise;
}

// Get PayPal ID for a user.
function getPaypal(userId) {
  const promise = userModel.findById(userId).select('paypal_id').catch((err) => console.log(err));
  return promise;
}

// Add a new rating for a driver or rider.
function addRating(userId, rating) {
  const promise = userModel.findByIdAndUpdate(userId, { $push: { ratings: rating } }, { new: true }).catch((err) => console.log(err));
  return promise;
}
